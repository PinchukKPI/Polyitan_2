...
T_DNS2 = [1 0 0; 0 0 -1; 0 1 0]; 
T_DNS3 = [1 0 0; 0 -1 0; 0 0 -1]; 
...
% 
T_GYRO = [0.00728902 -0.00503595 0.99996075; 0.99996051 0.00434607 0.00775122; -0.00340712 0.99998076 -0.00518317;];
T_L3G4200 = [-0.00571910 0.00335430 0.99997802; 0.00035278 -0.99999688 0.00247470; 0.99998127 -0.00170406 -0.00587767;];
% T_L3G4200 = [0.00728902 -0.00503595 0.99996075; 0.99996051 0.00434607 0.00775122; -0.00340712 0.99998076 -0.00518317;];
% T_GYRO = [-0.00571910 0.00335430 0.99997802; 0.00035278 -0.99999688 0.00247470; 0.99998127 -0.00170406 -0.00587767;];
%
...
d_alpha_b_s = 50; % град, Угол рассогласования между измеренным и расчетным конусом направления магнитного вектора и солнечного направления
% 
link_printout = 'D:\QB50\';
link_orbit = 'D:\SS\';
fid_data1 = fopen(strcat(link_printout,'res_MS2.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_graf.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_MS2_SUN1.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_MS2_SUN2.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_MS2_SUN3.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_graf_SUN1.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_graf_SUN2.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_graf_SUN3.txt'),'w');
fclose(fid_data1);
fid_data1 = fopen(strcat(link_printout,'res_graf_DUS.txt'),'w');
fclose(fid_data1);
%
printout_angl = 0;
printout_angl_velo = 1;
%
f_id_inputorbit = fopen(strcat(link_orbit,'POLYITAN-2-SAU.txt'),'r'); % Открытие файла *.txt
...
for i_names = 1:1:i_size(1)
    if strcmp(names(i_names),'ID1_SUN1_X') || strcmp(names(i_names),'POS_SunMach.X1')
        ID1_SUN1_X = i_names;
    elseif strcmp(names(i_names),'ID1_SUN1_Y') || strcmp(names(i_names),'POS_SunMach.Y1')
        ID1_SUN1_Y = i_names;
    elseif strcmp(names(i_names),'ID1_SUN2_X') || strcmp(names(i_names),'POS_SunMach.X2')
        ID1_SUN2_X = i_names;
    elseif strcmp(names(i_names),'ID1_SUN2_Y') || strcmp(names(i_names),'POS_SunMach.Y2')
        ID1_SUN2_Y = i_names;
    elseif strcmp(names(i_names),'ID1_SUN3_X') || strcmp(names(i_names),'POS_SunMach.X3')
        ID1_SUN3_X = i_names;
    elseif strcmp(names(i_names),'ID1_SUN3_Y') || strcmp(names(i_names),'POS_SunMach.Y3')
        ID1_SUN3_Y = i_names;
    elseif strcmp(names(i_names),'ID1_ADC_RTC_S')
        ID1_ADC_RTC_S = i_names;
    elseif strcmp(names(i_names),'POS_MagRtcTime.time_s')
        ID1_ADC_RTC_S = i_names - 1;
    elseif strcmp(names(i_names),'ID1_ADC_RTC_SS')
        ID1_ADC_RTC_SS = i_names;
    elseif strcmp(names(i_names),'POS_MagRtcTime.ss256')
        ID1_ADC_RTC_SS = i_names - 1;
    elseif strcmp(names(i_names),'ID1_MagTl_X') || strcmp(names(i_names),'POS_MagXnTl')
        ID1_MagTl_X = i_names;
    elseif strcmp(names(i_names),'ID1_MagTl_Y') || strcmp(names(i_names),'POS_MagYnTl')
        ID1_MagTl_Y = i_names;
    elseif strcmp(names(i_names),'ID1_MagTl_Z') || strcmp(names(i_names),'POS_MagZnTl')
        ID1_MagTl_Z = i_names;
    elseif strcmp(names(i_names),'ID1_GYRO_X_mdps')
        ID1_GYRO_X = i_names;
    elseif strcmp(names(i_names),'ID1_GYRO_Y_mdps')
        ID1_GYRO_Y = i_names;
    elseif strcmp(names(i_names),'ID1_GYRO_Z_mdps')
        ID1_GYRO_Z = i_names;
    elseif strcmp(names(i_names),'ID1_L3G4200_X_mdps')
        ID1_L3G4200_X = i_names;
    elseif strcmp(names(i_names),'ID1_L3G4200_Y_mdps')
        ID1_L3G4200_Y = i_names;
    elseif strcmp(names(i_names),'ID1_L3G4200_Z_mdps')
        ID1_L3G4200_Z = i_names;
    end
end

is_GYRO = 0;
if exist('ID1_GYRO_X') == 1 && exist('ID1_GYRO_Y') == 1 && exist('ID1_GYRO_Z') == 1
    is_GYRO = 1;
end

is_L3G4200 = 0;
if exist('ID1_L3G4200_X') == 1 && exist('ID1_L3G4200_Y') == 1 && exist('ID1_L3G4200_Z') == 1
    is_L3G4200 = 1;
end
...
while ischar(Meag_L_str)
    ...
    DateTime_meg = unixtime(str2double(Meag_L(ID1_ADC_RTC_S))+str2double(Meag_L(ID1_ADC_RTC_SS))*0.001);
    ...
    if is_GYRO == 1
        Meag(i_L1,16) = str2double(Meag_L(ID1_GYRO_X));
        Meag(i_L1,17) = str2double(Meag_L(ID1_GYRO_Y));
        Meag(i_L1,18) = str2double(Meag_L(ID1_GYRO_Z));
    end
    if is_L3G4200 == 1
        Meag(i_L1,19) = str2double(Meag_L(ID1_L3G4200_X));
        Meag(i_L1,20) = str2double(Meag_L(ID1_L3G4200_Y));
        Meag(i_L1,21) = str2double(Meag_L(ID1_L3G4200_Z));
    end
...
T_co_LAOO_before_i_s = zeros(3,3,3);
T_b = 0;
...
for i_meas = 1:1:i_size(1)
    ...
    if is_GYRO == 1
        GYRO_X = Meag(i_meas, 16);               % [mdeg/s]
        GYRO_Y = Meag(i_meas, 17);               % [mdeg/s]
        GYRO_Z = Meag(i_meas, 18);               % [mdeg/s]
        GYRO = T_GYRO*[GYRO_X GYRO_Y GYRO_Z]';
    else
        GYRO = [0 0 0]';
    end
    if is_L3G4200 == 1
        L3G4200_X = Meag(i_meas, 19);            % [mdeg/s]
        L3G4200_Y = Meag(i_meas, 20);            % [mdeg/s]
        L3G4200_Z = Meag(i_meas, 21);            % [mdeg/s]
        L3G4200 = T_L3G4200*[L3G4200_X L3G4200_Y L3G4200_Z]';
    else
        L3G4200 = [0 0 0]';
    end
    % -----------------------------------------------------------------------
...
%   
	...
	if IsMeas_S_1_MDA == 1
        S_meas = [S_meas S_1'];
        is_used_S_meas = [is_used_S_meas 1];
    end 
	if IsMeas_S_2_MDA == 1
        S_meas = [S_meas S_2'];   
        is_used_S_meas = [is_used_S_meas 2];
    end 
	if IsMeas_S_3_MDA == 1
        S_meas = [S_meas S_3'];   
        is_used_S_meas = [is_used_S_meas 3];
    end 
%
%     IsMeas_S_2 = norm(S_2);
%     IsMeas_S_3 = norm(S_3);
    ...
    if i_s_end >= 1 && i_b_end >= 1
        ...
        for i_b = 1:1:i_b_end
            for i_s = 1:1:i_s_end
                ...
                if norm(T_co_LAOO_before) == 0 || i_meas == 1
...
%        u_st_o = [0. sqrt(mu_z/r_^3.) 0.].';
        ...
        fid_data1 = fopen(strcat(link_printout,'res_MS2.txt'),'at+');
        ...
        fid_data1 = fopen(strcat(link_printout,'res_graf.txt'),'at+');
        fprintf(fid_data1,'%15.2f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f \n', T_e, ELIR_angle(Lamda_T(T_co_LAOO)), norm(w_cu_c_LAOO_rez*180/pi), w_cu_c_LAOO_rez*180/pi, B_meas/10^(-9), L3G4200, GYRO);
        fclose(fid_data1);
        
        % Отдельно по каждому СД
        for i_b = 1:1:i_b_end
            for i_s = 1:1:i_s_end
                B_c_i = B_meas((i_b*3-2):i_b*3)';
                S_c_i = S_meas((i_s*3-2):i_s*3)';
                T_co_LAOO = LAOO(B_c_i, B_o_print, S_c_i, S_o_print);
                if norm(T_co_LAOO_before_i_s(:,:,i_s)) == 0 || i_meas == 1
                    T_co_LAOO_before_i_s(:,:,i_s) = T_co_LAOO;
                    T_control = 1;
                else
                    t_i = unixtime([Meag(i_meas, 1) Meag(i_meas, 2) Meag(i_meas, 3) Meag(i_meas, 4) Meag(i_meas, 5) Meag(i_meas, 6)]);
                    t_i_1 = unixtime([Meag(i_meas-1, 1) Meag(i_meas-1, 2) Meag(i_meas-1, 3) Meag(i_meas-1, 4) Meag(i_meas-1, 5) Meag(i_meas-1, 6)]);
                ...
                T_co_LAOO_st = (T_co_LAOO - T_co_LAOO_before_i_s(:,:,i_s))/(T_control);
                ...
                T_co_LAOO_before_i_s(:,:,i_s) = T_co_LAOO;
                fid_data1 = fopen(strcat(link_printout,'res_MS2_SUN',int2str(is_used_S_meas(i_s)),'.txt'),'at+');
                fprintf(fid_data1,'| Y M D h m s = %5.0f %3.0f %3.0f %3.0f %3.0f %3.0f | TETA FI PSI [deg] = %7.3f %7.3f %7.3f | w_cu_c_LAOO_rez [deg/sec] = %7.3f %7.3f %7.3f | Grad_calc [deg] = %7.3f | Grad_meas1 dGrad1 [deg] = %7.3f %7.3f | Grad_meas2 dGrad2 [deg] = %7.3f %7.3f | Grad_meas3 dGrad3 [deg] = %7.3f %7.3f |\n', DateTime_meg, ELIR_angle(Lamda_T(T_co_LAOO)), w_cu_c_LAOO_rez*180/pi, grad_S_o_calc_B_o_calc, grad_S_1_MDA, (grad_S_o_calc_B_o_calc - grad_S_1_MDA), grad_S_2_MDA, (grad_S_o_calc_B_o_calc - grad_S_2_MDA), grad_S_3_MDA, (grad_S_o_calc_B_o_calc - grad_S_3_MDA));
                fclose(fid_data1);
  
                fid_data1 = fopen(strcat(link_printout,'res_graf_SUN',int2str(is_used_S_meas(i_s)),'.txt'),'at+');
                fprintf(fid_data1,'%15.2f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f \n', T_e, ELIR_angle(Lamda_T(T_co_LAOO)), norm(w_cu_c_LAOO_rez*180/pi), w_cu_c_LAOO_rez*180/pi, B_meas/10^(-9), L3G4200, GYRO);
                fclose(fid_data1);
...
%         if exist('grad_S_o_calc_B_o_calc') == 0
%             grad_S_o_calc_B_o_calc = 0;
%         end
%         
%         fid_data1 = fopen(strcat(link_printout,'res_MS2.txt'),'at+');
...
%         
%         fid_data1 = fopen(strcat(link_printout,'res_graf.txt'),'at+');
%         fprintf(fid_data1,'%15.2f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f \n', T_e, 0, 0, 0, 0, 0, 0, 0, B_meas/10^(-9), L3G4200, GYRO);
%         fclose(fid_data1);
    end
    
    fid_data1 = fopen(strcat(link_printout,'res_graf_DUS.txt'),'at+');
    fprintf(fid_data1,'%15.2f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f\n', T_e, L3G4200, GYRO);
    fclose(fid_data1);
    ...
    if printout_angl == 1
        data_ = load(strcat(link_printout,'res_graf.txt'));
        time = data_(:,1);
        TETA = data_(:,2);
        FI = data_(:,3);
        KSI = data_(:,4);
        figure
        plot(time(:),TETA(:),'r-',time(:),FI(:),'b-',time(:),KSI(:),'g-')
        grid on
        title('Углы ориентации МС относительно ОСК')
        xlabel('Время, милисек')
        ylabel('Значение углов ориентации, град')
        legend('TETA', 'FI', 'KSI')
    end
    if printout_angl_velo == 1
        data_ = load(strcat(link_printout,'res_graf.txt'));
        time = data_(:,1);
        normw = data_(:,5);
        wx = data_(:,6);
        ...
        wz = data_(:,8);
        figure
        plot(time(:),normw(:),'k-', time(:),wx(:),'r-',time(:),wy(:),'b-',time(:),wz(:),'g-')
        grid on
        title('Угловая скорость МС')
        xlabel('Время, милисек')
        ylabel('Значение угловой скорости, град/сек')
        legend('|w|','wx', 'wy', 'wz')
    end
